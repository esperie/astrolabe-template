...
```bash
ls -t ./readings/*.md 2>/dev/null | head -8 || echo "  (no readings yet)"
```
...
```bash
grep -A3 "^## 4\." "./.claude/canon/canon.md" 2>/dev/null | grep -iE "用神|忌神" | head -4 || echo "  (no canon §4 — use the framework default palette)"
```
## What to do
...
4. **Render:**
   ```
   node "./.claude/bin/render-reading.mjs" <reading.md> \
