...
for (const [y, m, d] of [[1900, 3, 21], [1950, 3, 21], [2000, 3, 20], [2100, 3, 20]]) {
...
// 大雪 regression (winter term seeded via the next year per the autumn/winter convention)
const dx = astro.jdnToGregorian(Math.floor(astro.solarTermUT(2001, 255) + 0.5));
ok("大雪 2000 ≈ Dec 7", dx.y === 2000 && dx.m === 12 && Math.abs(dx.d - 7) <= 1, `${dx.y}-${dx.m}-${dx.d}`);
