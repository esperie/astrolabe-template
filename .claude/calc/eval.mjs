...
const META = {
  ...
  "lane.test.mjs":             { system: "cross-CLI lane contract", expect: 35,  oracle: "lane-native hook payloads (codex/gemini) through a real subprocess", residual: "Codex 0.147 invokes NO hooks (measured, repo-local + CODEX_HOME); Gemini lane unverified live (account tier) — see rules/multi-cli.md" },
  ...
  "dasha-margin.test.mjs":     { system: "daśā date margin",        expect: 27, oracle: "vimshottari() re-derived independently inside the suite, then swept across a stated admissible birth window — the tool polices CLAIMS, not ephemeris", residual: "the window width is inferred only for cert/rounded minutes and REFUSES on a recollection-shaped time (:00/:30), which is a convention, not a fact about any birth record; both verdict ladders (3d/21d/90d and 10%/50%/100%) are a CHOSEN scale, disclosed in the tool's own output; the raw day-spread is IDENTICAL at every daśā level by construction — spread÷period is the level-dependent quantity, and gating on the raw spread alone was the original design bug; 'which lord is running' and 'when it changes' are separate questions and both are reported; fixtures are synthetic so the suite carries no subject data" },
};
